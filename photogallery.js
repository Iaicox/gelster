var projectsList = document.getElementsByClassName('photo_project'),
    ...
    prevPhoto, nextPhoto, i, j,
    curFolder = gallery.children[0].src.split('pages-name-imgs/photogallery/')[1].split('/')[0],
...
function changeProject(event) {
    var projectNum, i = 0,
        checkId = true,
        mainParent, curImg = [],
        projectsImgSrc, fullProjectsImgSrc;
    var path = event.path || (event.composedPath && event.composedPath()) || iePath(event.target);
    ...
    bigPhoto.style.backgroundImage = 'url("./img/pages-name-imgs/photogallery/' + curFolder + '/' + projectNum + '-1.jpg")';

    checkId = true;
    i = 1;

    while (checkId) {
        fullProjectsImgSrc = './img/pages-name-imgs/photogallery/' + curFolder + '/' + projectNum + '-' + i + '.jpg';

        curImg[i] = document.createElement('img');
        curImg[i].src = './img/pages-name-imgs/photogallery/' + curFolder + '/' + projectNum + '-' + i + '.jpg';
...
function changeMainPhoto(direction) {
    if (direction === 'next') {
        ...
        bigPhoto.style.backgroundImage = 'url("' + gallery.children[nextPhoto].src + '")';
    ...
    function scrollGallery(direction, times) {
        if (direction === 'next') {
            gallery.scrollLeft += times * (($(".gallery-cur_project")[0].offsetWidth / 5) + 3);
        } else {
            gallery.scrollLeft += times * (-1) * (($(".gallery-cur_project")[0].offsetWidth / 5) + 3);
        }
    }
}
...
function fixGridIe() {
    var allProjects = document.getElementsByClassName('projects_lib-item'),
        ieProject, ieProjectNum, gridRowNum = 3,
        gridCol, gridColNum = 1;
    i = 0, j = 0;
