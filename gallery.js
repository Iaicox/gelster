...
(function($) {
  ...
  AdGallery.prototype = {
    ...
    _slideshowNext: function() {
      ...
      this.slideshow_countdown.show().html(pre + (this.settings.slideshow.speed / 1000) +' сек.' + su);
      var slide_timer = 0;
        ...
        function() {
          context.slideshow_countdown_interval = setInterval(
            function() {
              ...
              sec--;
              if(sec > 0) {
                context.slideshow_countdown.html(pre + sec +' сек.' + su);
              };
            },
            1000
          );
    ...
    _afterShow: function() {
      this.gallery_info.html('<span style="font-family:sans-serif">Фото: ' + '<span style="color:green">' + (this.current_index + 1) +'</span> из <span style="color:red">'+ this.images.length +'</span>');
